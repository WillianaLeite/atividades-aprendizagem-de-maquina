...
class NormalUnivariate():
    ...
    def plot_pdf(self, x=None):
        ...
        plt.title(f'Media = {self.mean},  std = {self.std}')
        plt.show()
...
class NormalMultivariate():
    ...
    def plot_pdf(self, col_x1, col_x2):
        ...
        plt.title(f'Media = {self.mean},  cov = {self.cov}')
        plt.show()
...
class ClassifierBayes():
    ...
    def predict_proba(self, df):
        ...
        for idx, row in df.iterrows():
            ...
            normalize = sum(pdf_priori)
            if len(self.list_class) == 2: # Binary
                proba =  (dict_pdf[0] * self.dict_priori[0]) / normalize
                dict_pred_classes['proba_0'].append(proba)
                dict_pred_classes['proba_1'].append(1 - proba)
            else:
                for j in sorted(self.list_class): 
                    proba =  (dict_pdf[j] * self.dict_priori[j]) / normalize
